# Applying technical indicators to football odds
...
# Now create a column showing returns for betting on the home team

team_nums <- all_data %>% 
  count(home_team) %>% arrange(n) %>% 
  filter(n > 50) %>% 
  select(home_team)

all_data <- all_data %>% 
  inner_join(team_nums) %>% 
  arrange(home_team, date) %>% 
  mutate(home_win = ifelse(ftr == "H", b365h - 1, -1)) %>%
  group_by(home_team) %>% 
  mutate(cum_home_perf = cumsum(home_win),
         cum_hg = cumsum(fthg)) %>% 
  mutate(home_game_num = row_number()) %>% 
  mutate(ema_2 = EMA(cum_home_perf, 2),
         ema_5 = EMA(cum_home_perf, 5),
         ema_8 = EMA(cum_home_perf, 8),
         ema_10 = EMA(cum_home_perf, 10),
         macd = MACD(cum_home_perf)[,1],
         macd_sig = MACD(cum_home_perf)[,2],
         lag_macd = lag(macd),
         lag_macd_sig = lag(macd_sig))

# bet if MACD positive

bets <- all_data %>% 
  ungroup() %>% 
  mutate(bet_amt = case_when(is.na(lag_macd) ~ 0,
                             lag_macd > lag_macd_sig ~ 1,
                             TRUE ~ 0),
         winner = ifelse(ftr == "H", 1, 0),
         win_amt = case_when(bet_amt == 1 & winner == 1 ~ b365h - 1,
                             bet_amt == 1 & winner == 0 ~ - 1,
                             TRUE ~ 0))
...
bets_numbered <- bets %>% 
  ...
  ungroup() %>%  
  arrange(date) %>% 
  mutate(rownunm = row_number(),
         cumulative_win_amt = cumsum(win_amt))

ggplot(bets_numbered,
       aes(x = rownunm, y = cumulative_win_amt, colour = season_id)) +
  geom_line()
