...
description: |
  Using the {rvest} package to scrape local election data, and {ggplot} to replicate some of the Guardian graphs.
author:
  - name: Nick Zani
    url: {}
date: 2023-05-09
...
p1 <- table_list_flat %>% 
    filter(name == council_name & share != 0) %>% 
      ...
      annotate("text",  x=1.7, y = totals, label = control_text, vjust=1, hjust=1, size = 6) +
      coord_flip() +      
      ...
      labs(title = council_name, 
           ...
           x = "",
           y = "") +
      theme(plot.title = element_text(hjust = 0.057, vjust = -7, margin=margin(0,0,3,0), size = 20),
            plot.subtitle = element_text(hjust = 0.057, vjust = -7, size = 16))
